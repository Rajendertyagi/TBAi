 ...
 * assistant-ui/elements/context-display.aui.tsx` — verbatim. Wires the
 * standalone presets in `./context-display` to the thread runtime
 * (`useThreadTokenUsage` + thread id as reset key). No TBAi adaptation.
 */
"use client";
...
function createWiredPreset(Preset: FC<PresetProps>): FC<WiredPresetProps> {
  const WiredPreset: FC<PresetProps> = (props) => {
    const usage = useThreadTokenUsage();
    const threadId = useAuiState((s) => s.threadListItem.id);
