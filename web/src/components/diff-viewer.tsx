...
/**
 * Line-based fallback for fences that are not valid unified patches
 * (bare +/-/space lines without ---/+++/@@ headers). Classifies each line by
 * its leading marker so models' informal diffs still render as diffs.
 */
function parseLooseDiff(patch: string): ParsedFile {
  ...
  for (const raw of patch.split("\n")) {
    if (raw.startsWith("+") && !raw.startsWith("+++")) {
      ...
      deletions++;
      lines.push({ type: "del", content: raw.slice(1), oldLineNumber: oldLine++ });
...
const diffViewerVariants = cva(
  "aui-diff-viewer w-full overflow-hidden font-mono leading-relaxed [font-variant-ligatures:none]",
  {
    variants: {
      variant: {
        default:
...
function DiffViewer({
  ...
  const parsedFiles = useMemo<ParsedFile[]>(() => {
    if (diffPatch) {
      const parsed = parsePatch(diffPatch);
      // Models often emit bare +/- lines without unified-diff headers, which
      // parse-diff rejects. Fall back to line-based add/del classification so
      // the fence still renders as a diff instead of collapsing.
      if (parsed.length > 0) return parsed;
      const fallback = parseLooseDiff(diffPatch);
