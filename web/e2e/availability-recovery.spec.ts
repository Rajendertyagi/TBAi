...
test("outage retains stale state and gates send, recovery refreshes authoritatively", async ({
  ...
  // Hermetic: this spec seeds a REAL row in the shared database, so it has to
  // remove it. Without the finally it leaked one "Known Conversation" per run,
  // which then broke the count assertion on every later run - a spec that
  // guaranteed its own second-run failure.
  try {
    ...
    // THIS conversation survived the outage, asserted by identity. The old
    // `toHaveLength(1)` asserted the entire database held exactly one row,
    // which is a claim about every other spec and about real use - it could
    // never be stable on a shared database, and it is not what this test is
    // about. Retention is already proven above by the visible row.
    expect(data.threads.map((t) => t.id)).toContain(conversationId);
  } finally {
    await page.request
